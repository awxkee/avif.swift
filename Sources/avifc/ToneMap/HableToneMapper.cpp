...
float32x4_t HableToneMapper::Execute(const float32x4_t m) {
    const float A = 0.15, B = 0.50, C = 0.10, D = 0.20, E = 0.02, F = 0.30;
    const float32x4_t den = vaddq_f32(vmulq_f32(m, vaddq_f32(vmulq_n_f32(m, A), vdupq_n_f32(C*B))), vdupq_n_f32(D*E));
    const float32x4_t num = vaddq_f32(vmulq_f32(vmlaq_f32(vdupq_n_f32(B), m, vdupq_n_f32(A)), m), vdupq_n_f32(D*F));
